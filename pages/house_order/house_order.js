...
Page({
  ...
  onLoad: function (option) {
    ...
    wx.request({
      url: 'http://localhost:8088//homestay_servlet_room_order_servlet_action?action=get_room_order_record',
  ...
  paySpecialtyOrder: function (e) {
    ...
    wx.request({
      ...
      data:{"good_list":"["+JSON.stringify(room)+"]","action":"pay_order","good_type":"house"},
      method:"POST",
      header:{"content-type":"application/x-www-form-urlencoded","x-requested-with":"XMLHttpRequest",'cookie':wx.getStorageSync("token")},
      success:function(res){
        if(res.data.code==0)
